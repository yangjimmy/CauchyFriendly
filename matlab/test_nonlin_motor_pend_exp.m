...
Ts = data.t; % 1000 Hz
% mp.dt = mean(Ts(1:end-1)-Ts(1:end-1));
% mp.sr = 1/mp.dt;
zs = data.pos;
...
%     plot(Ts(1:end-1), sqrt(cauchyEst.moment_info.P(:,idx,idx)), c_std_color); hold on;
%     plot(Ts(1:end-1),-sqrt(cauchyEst.moment_info.P(:,idx,idx)), c_std_color); hold on;
...
c_e = cauchyEst.moment_info.x(:,idx) - data.pos(1:end-1);
...
plot(Ts, -kf_std,k_std_color); hold on;
plot(Ts(1:end-1), c_e, c_err_color); hold on;
plot(Ts(1:end-1),  c_std, c_std_color); hold on;
plot(Ts(1:end-1), -c_std, c_std_color); hold on;
grid on; 
...
c_e = cauchyEst.moment_info.x(:,idx) - data.vel(1:end-1);
kf_std = sqrt(Ps_kf(:,idx,idx));
c_std = sqrt(cauchyEst.moment_info.P(:,idx,idx));
...
plot(Ts(1:end-1), cauchyEst.moment_info.x(:,1),'linewidth',1.5); hold on;
...
plot(Ts(1:end-1), cauchyEst.moment_info.x(:,2),'linewidth',1.5); hold on;
